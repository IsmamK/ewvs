    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 px-8 md:px-16 lg:px-24">
        {displayReasons.map((reason, index) => (
          <div 
            key={index} 
            className="bg-white backdrop-blur-lg p-8 rounded-xl shadow-2xl transform transition  duration-300 ease-in-out text-center"
          >
            <div className="mb-4">{reason.icon}</div>
            <h3 className="text-2xl font-semibold text-ewvs mb-2">{reason.title}</h3>
            <p className="text-ewvs text-opacity-80">{reason.description}</p>
          </div>
        ))}
      </div>
